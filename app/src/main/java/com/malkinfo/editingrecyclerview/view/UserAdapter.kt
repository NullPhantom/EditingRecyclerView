              ...
              when(it.itemId){
                      ...
                      val number = v.findViewById<EditText>(R.id.userNo)
                              AlertDialog.Builder(c)
                                      ...
                                      .setPositiveButton("Ok"){
                                          dialog,_->
                                          position.userName = "Estado: ${name.text.toString()}"
                                          position.userMb = "ID: ${number.text.toString()}"
                                          notifyDataSetChanged()
                                          Toast.makeText(c,"Estado actualizado",Toast.LENGTH_SHORT).show()
                                          dialog.dismiss()
                  ...
                  R.id.delete->{
                      ...
                      AlertDialog.Builder(c)
                              ...
                              .setMessage("¿Estas seguro que deseas eliminar el estado?")
                              .setPositiveButton("Si"){
                                  dialog,_->
                                  userList.removeAt(adapterPosition)
                                  notifyDataSetChanged()
                                  Toast.makeText(c,"Estado eliminado",Toast.LENGTH_SHORT).show()
                                  dialog.dismiss()
                              }
